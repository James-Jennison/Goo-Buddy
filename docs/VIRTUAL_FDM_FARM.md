...
Start it with `docker compose -f docker-compose.virtual-farm.yml --profile virtual-farm up --abort-on-container-exit`; stop it with `down`. It contains only a narrow Moonraker read-only simulator: fixed monitoring requests, camera metadata, a fixed `gcodes` inventory, and cached console history. The inventory accepts only `GET /server/files/list?root=gcodes`; console history accepts only `GET /server/gcode_store?count=50`. Both supply deterministic display data and neither provides file transfer or command control. Bambu and SDCP contract coverage is exercised by their existing Goo Buddy adapter fixtures; no proprietary protocol is guessed or exposed.
