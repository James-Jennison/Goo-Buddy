...
Start it with `docker compose -f docker-compose.virtual-farm.yml --profile virtual-farm up --abort-on-container-exit`; stop it with `down`. It contains only a narrow Moonraker read-only simulator. Bambu and SDCP contract coverage is exercised by their existing Goo Buddy adapter fixtures; no proprietary protocol is guessed or exposed.
