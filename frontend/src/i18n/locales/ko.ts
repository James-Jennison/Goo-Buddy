export default {
  nav: {
    ...
    logout: '로그아웃',
    installApp: '앱 설치',
    installAppSuccess: 'Goo Buddy가 설치되었습니다'
  },
  common: {
    save: '저장',
    saving: '저장 중...',
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldHelp: '활성화하면 이 프린터는 MQTT, 대기열 디스패치 및 알림에서 일시 중지됩니다 — 수리, 병렬 Goo Buddy 설치 또는 임시 중지에 유용합니다.',
    },
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: '슬라이서가 프린터의 SD 카드에 .gcode.3mf 파일을 남기지 않아서 Goo Buddy가 썸네일이나 슬라이서 메타데이터를 가져올 수 없었습니다. 일반적으로 슬라이서 (Bambu Studio / OrcaSlicer 장치 탭)에서 "전송된 파일을 외부 저장소에 저장"이 꺼져 있을 때 발생합니다.',
      docsLink: '설치 단계 4 보기',
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: 'LDAP 그룹이 로그인 시 Goo Buddy 그룹에 매핑됨',
      ...
      autoProvisionHint: '첫 LDAP 로그인 시 자동으로 Goo Buddy 계정 생성',
      defaultGroup: '기본 그룹',
      ...
      groupMappingHint: 'LDAP 그룹 DN을 Goo Buddy 그룹에 매핑합니다. 사용 가능한 그룹: ',
    ...
    concurrentUploadsDescription: '대기열이 동시에 파일을 보낼 수 있는 프린터 수입니다. 프린터는 파일을 느리게 받으며(큰 출력물은 몇 분이 걸릴 수 있습니다) 각 프린터는 자기 차례를 기다립니다. 따라서 프린터가 많은 환경에서는 이 값을 높이면 배치의 마지막 프린터가 앞선 모든 전송이 끝나기를 기다리지 않아도 됩니다. 네트워크나 Goo Buddy 호스트가 병렬 전송을 버거워하면 값을 낮추세요.',
    ...
    preheatFilamentTargetsHint: 'Goo Buddy는 로드된 AMS 슬롯 중 가장 높은 목표를 선택합니다. PLA만 사용하는 인쇄는 0으로 도출되어 챔버 단계를 자동으로 건너뜁니다.',
    ...
    authDisabledMessage: '사용자 계정 생성, 권한 관리, Goo Buddy 인스턴스 보안을 위해 인증을 활성화하세요.',
    ...
    disableAuthenticationMessage: '인증을 비활성화하시겠습니까? 이렇게 하면 로그인 없이 Goo Buddy 인스턴스에 접근할 수 있게 됩니다. 모든 사용자는 데이터베이스에 유지되지만 인증은 비활성화됩니다.',
    disableAuthentication: '인증 비활성화',
    configureBambuddy: 'Goo Buddy 설정',
    ...
    captureFinishPhotoDescription: '인쇄 완료 시 프린터 카메라로 사진 촬영. Goo Buddy는 인쇄 중 짧은 타임랩스를 기록하여 베드가 내려가기 전 순간에서 사진을 가져올 수 있도록 합니다. 이 인쇄에 대해 타임랩스를 활성화한 경우 타임랩스 파일이 보관되며, 그렇지 않으면 사진 촬영 후 자동으로 삭제됩니다.',
    ...
    bambuddySoftware: 'Goo Buddy 소프트웨어',
    autoCheckDescription: '시작 시 자동으로 새 버전 확인',
    checkNow: '지금 확인',
    updateAvailableVersion: '업데이트 가능: v{{version}}',
    releaseNotes: '릴리스 노트',
    updateViaDocker: 'Docker Compose로 업데이트:',
    updateViaHomeAssistant: '업데이트는 Home Assistant 수퍼바이저에서 관리됩니다. Home Assistant에서 설정 → 애드온 → Goo Buddy를 열어 새 버전을 설치하세요.',
    ...
    goToBackup: '백업으로 이동',
    externalUrl: '외부 URL',
    externalUrlDescription: 'Goo Buddy에 접근할 수 있는 외부 URL. 알림 이미지 및 외부 통합에 사용됩니다.',
    ...
    mqttDescription: 'Node-RED, Home Assistant 등 자동화 시스템과 통합하기 위해 Goo Buddy 이벤트를 외부 MQTT 브로커에 게시하세요.',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP 시크릿 및 OIDC client_secret이 일반 텍스트로 저장됩니다. MFA_ENCRYPTION_KEY를 설정하거나 쓰기 가능한 데이터 디렉토리로 Goo Buddy를 재시작하여 자동 생성하세요.',
      ...
      migrationErrorWarning: '시작 시 {{count}}개 레거시 행을 재암호화하지 못했습니다. 서버 로그를 확인하고 Goo Buddy를 재시작하여 재시도하세요.'
    },
  ...
  hmsErrors: {
    ...
    runoutSlotUnknown: '필라멘트가 소진되어 인쇄가 일시정지되었습니다. Goo Buddy가 프린터가 현재 요청하는 슬롯을 확인할 수 없습니다 — 프린터 화면에서 요청된 슬롯을 확인하세요.',
    actions: {
  ...
  login: {
    ...
    resetStep1: 'Goo Buddy 관리자에게 문의하세요',
    ...
    twoFA: {
      ...
      methodEmail: '이메일 코드',
      methodBackup: '백업 코드',
      instructionsTotp: '인증 앱을 열고 Goo Buddy의 6자리 코드를 입력하세요.',
  ...
  setup: {
    title: 'Goo Buddy 설정',
    subtitle: 'Goo Buddy 인스턴스의 인증을 설정하세요',
    enableAuth: '인증 활성화',
    adminAccount: '관리자 계정',
  ...
  users: {
    title: '사용자 관리',
    subtitle: 'Goo Buddy 인스턴스에 대한 사용자 및 접근 권한 관리',
    backToSettings: '설정으로 돌아가기',
    createUser: '사용자 생성',
    noPermission: '이 페이지에 접근할 권한이 없습니다.',
    admin: '관리자',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        ...
        description: 'Orca Cloud 계정에 로그인하여 슬라이서 프로필을 Goo Buddy에 동기화하세요.',
      },
      device: {
        title: 'Orca Cloud에서 Goo Buddy 승인',
        instruction: 'Orca Cloud를 열고 이 코드를 승인하세요. 승인하면 Goo Buddy가 자동으로 연결됩니다.',
        codeLabel: '페어링 코드',
  ...
  sponsors: {
    ...
    tagline: 'Goo Buddy는 무료이며, 자발적으로 후원하는 사용자 덕분에 계속 무료로 유지됩니다. VC도 없고, 클라우드 종속도 없습니다.',
    viewSupporters: '후원자 보기',
    toastPrints: 'Goo Buddy로 {{count}}회 인쇄를 완료했습니다. Goo Buddy는 후원자 덕분에 무료로 유지됩니다.',
    toastCost: 'Goo Buddy로 {{total}}만큼의 필라멘트를 추적했습니다. 프로젝트를 독립적으로 유지하는 사람들을 만나보세요.',
    toastArchives: 'Goo Buddy로 {{count}}회 인쇄를 아카이브했습니다. 독립성을 지지하는 사람들을 만나보세요.',
    toastAnniversary: 'Goo Buddy와 함께한 지 1년입니다! 프로젝트를 독립적으로 유지하는 사람들을 만나보세요.',
    toastVersionUpdate: 'v{{version}}로 업데이트되었습니다. Goo Buddy는 후원자 덕분에 무료로 유지됩니다.',
    toastBusiness: '프린터 {{count}}대에서 Goo Buddy를 운영 중이신가요? 팀을 위한 지원 플랜이 있습니다. 우선 수정, 인보이스 발행, 메인테이너와의 직접 소통을 제공합니다.',
    businessCta: '비즈니스용 Goo Buddy',
    businessTitle: '비즈니스용 Goo Buddy',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'Goo Buddy가 HTTPS로 서비스되고 있지만 Spoolman URL은 여전히 HTTP입니다. 브라우저는 보안상 혼합 콘텐츠를 차단하므로 내장된 Spoolman UI가 렌더링되지 않습니다. 이 기능이 작동하려면 Spoolman이 HTTPS로 접근 가능해야 합니다.',
    spoolmanMixedContentFixReverseProxy: 'Spoolman을 Goo Buddy와 같은 리버스 프록시(Traefik / Nginx / Caddy) 뒤에 HTTPS로 배치한 다음 설정에서 Spoolman URL을 새 HTTPS 주소로 업데이트하세요.',
  ...
  backup: {
    ...
    appSettings: '앱 설정',
    appSettingsDescription: 'Goo Buddy 구성 (전체 데이터베이스)',
    ...
    commit: '커밋',
    localBackup: '로컬 백업',
    localBackupDescription: '데이터베이스, 아카이브, 업로드 및 모든 파일을 포함한 Goo Buddy 데이터의 전체 백업을 만듭니다.',
    ...
    backupRestoredRestart: '백업 복원됨. Goo Buddy를 재시작하세요.',
    ...
    pathCheck: {
      title: 'Goo Buddy가 이 디렉터리에 쓸 수 없습니다',
      howToFix: '해결 방법:',
      sandboxed: 'Goo Buddy 서비스가 {{path}}에 쓸 수 없습니다. systemd 유닛이 ProtectSystem=strict로 실행되어 설치·데이터·로그 디렉터리를 제외한 모든 디렉터리가 서비스에서는 읽기 전용입니다. 사용자 셸에서는 쓸 수 있는 디렉터리라도 마찬가지입니다.',
      read_only: '{{path}}이(가) 읽기 전용 파일 시스템에 있습니다.',
      permission_denied: 'Goo Buddy에 {{path}} 쓰기 권한이 없습니다. 디렉터리 소유자와 권한을 확인하세요.',
      no_space: '{{path}}이(가) 있는 파일 시스템에 남은 공간이 없습니다.',
      not_a_directory: '{{path}}이(가) 존재하지만 디렉터리가 아닙니다.',
      missing: '{{path}}이(가) 존재하지 않으며 생성할 수도 없습니다.',
      error: 'Goo Buddy가 {{path}}에 쓸 수 없습니다.',
      ephemeralTitle: '이 백업은 컨테이너를 다시 만들면 사라집니다',
      container_ephemeral: '{{path}}은(는) 호스트가 아니라 Goo Buddy 컨테이너 내부에 있습니다. 여기에 기록된 백업은 컨테이너를 재생성하면 사라집니다. 호스트의 디렉터리를 마운트하세요:',
    },
    categories: {
      settings: '설정',
    ...
    repoIsPublicWarning: '저장소가 공개입니다. Goo Buddy 백업에는 MQTT 자격증명, Home Assistant 토큰, Prometheus 토큰, Bambu Cloud 이메일, K-프로파일을 통한 프린터 액세스 코드가 포함됩니다. 공급자 설정에서 저장소를 비공개로 변경할 때까지 저장이 차단됩니다.',
    repoVisibilityUnknown: '저장소 공개 여부를 확인할 수 없습니다. Goo Buddy는 비공개로 확인된 저장소에만 백업하며, 저장이 차단됩니다.'
  },
  tags: {
    title: '태그',
    addTag: '태그 추가',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '메모는 프린터가 아닌 Goo Buddy에 저장됩니다',
      syncing: '프린터와 동기화 중...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '가상 프린터 활성화',
      visibleInSlicer: '슬라이서 검색에서 "Goo Buddy"로 표시됨',
    ...
    remoteInterface: {
      ...
      hint: 'SSDP를 통해 광고되고 TLS 인증서에 사용되는 IP 주소를 재정의합니다. Goo Buddy에 여러 네트워크 인터페이스가 있을 때 유용합니다.'
    },
    mode: {
      title: '모드',
      archive: '아카이브',
    ...
    howItWorks: {
      ...
      step2: '아카이브, 검토, 대기열 모드에서 슬라이서의 "전송" 버튼을 사용하여 3MF 파일을 Goo Buddy에 업로드하세요. 슬라이서는 "인쇄 성공"을 표시하지만 파일은 저장되고 인쇄되지 않습니다.',
    ...
    caCert: {
      title: '슬라이서 인증서',
      description: '가상 프린터는 Goo Buddy CA가 서명한 TLS 인증서를 사용합니다. 이 CA 인증서를 슬라이서의 신뢰 저장소에 한 번만 가져오면 연결을 수락합니다 — 명령줄에서 복사할 필요가 없습니다.',
      copy: '복사',
      copied: '복사됨',
      download: '다운로드',
      fingerprint: 'SHA-256'
  ...
  smartPlugs: {
    ...
    restEnergyTotalHint:
      '많은 플러그가 — Shelly는 모두 — 초기화되지 않는 누적 카운터만 보고합니다. 그 값은 위 항목이 아니라 여기에 입력하세요. 오늘 사용량으로 읽으면 자정에 초기화되지 않고, 어제와 총계는 계속 비어 있게 됩니다. Goo Buddy가 누적값에서 오늘과 어제를 계산하며, 이를 위해 하루에서 이틀 치 측정값이 필요합니다. Shelly는 와트시로 보고하므로 배율은 0.001을 사용하세요.',
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML API가 URL을 통해 카메라 스냅샷을 가져옵니다. ML API 컨테이너가 Goo Buddy에 도달할 수 있도록 일반 설정에서 외부 URL을 설정하세요.',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: 'MakerWorld 모델 URL을 붙여넣어 Bambu Handy 앱을 떠나지 않고 Goo Buddy에서 직접 가져오고 인쇄하세요.',
    ...
    signInExpiredTitle: 'Bambu 클라우드 로그인이 만료되었습니다',
    signInExpiredBody: 'Goo Buddy에는 여전히 로그인되어 있지만 Bambu Lab이 저장된 토큰을 더 이상 허용하지 않아 다운로드가 실패합니다. Bambu 클라우드에 다시 로그인하세요.',
    ...
    disclaimer: 'MakerWorld 통합은 커뮤니티가 문서화한 API 엔드포인트를 사용합니다. Goo Buddy는 MakerWorld 또는 Bambu Lab과 제휴하거나 이들의 승인을 받지 않았습니다.',
  ...
  libraryPurge: {
    ...
    effect4: '외부(연결된) 폴더의 파일은 건너뜁니다 — Goo Buddy는 자신이 소유하지 않은 바이트는 절대 삭제하지 않습니다.',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'Goo Buddy가 Docker 브리지 네트워킹으로 실행 중입니다. 프린터 검색과 가상 프린터에는 호스트 네트워크 모드가 필요합니다 — "network_mode: host"로 컨테이너를 재생성하세요.',
        skip: 'Docker에서 실행 중이 아닙니다 — 해당 없음.'
      },
      subnet: {
        title: '네트워크 서브넷',
        pass: '프린터와 Goo Buddy가 같은 서브넷에 있습니다.',
        warn: '프린터({{printer_ip}})와 Goo Buddy({{host_ip}})가 다른 서브넷에 있습니다. 서브넷 간 라우팅이 구성되어 있지 않으면 서로 연결하지 못할 수 있습니다.',
  ...
  systemHealth: {
    ...
    signature: {
      "ftp-auth-rejected": {
        ...
        fix: '프린터 화면(LAN 설정)에서 액세스 코드를 다시 복사하여 Goo Buddy의 프린터 설정에서 업데이트하세요.'
      },
      "ftp-connection-timeout": {
        name: '파일 전송 연결 시간 초과',
        cause: 'Goo Buddy가 프린터의 파일 전송 포트(FTPS 990)에 연결할 수 없었습니다. 포트가 차단됐거나 프린터가 꺼져 있거나 다른 서브넷에 있습니다.',
        fix: 'Goo Buddy와 프린터 사이에서 포트 990을 차단하는 것이 없는지 확인하고, 같은 네트워크에 있는지 확인하세요.'
      },
      "ftp-ssl-error": {
      ...
      "database-locked": {
        ...
        fix: 'Goo Buddy를 외부 PostgreSQL 데이터베이스로 전환하세요. 설명서의 PostgreSQL 가이드를 참고하세요.'
      }
    }
  },
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: '서비스 실행 중',
        fail: '가상 프린터가 활성화됐지만 서비스가 실행 중이 아닙니다. Goo Buddy 로그를 확인하세요 — 바인드 IP 충돌이나 권한 오류가 보통 원인입니다.'
      },
      bind_interface: {
      ...
      certificate: {
        title: 'TLS 인증서',
        pass: '인증서가 준비됐습니다. Goo Buddy CA 인증서(위)가 슬라이서의 신뢰 저장소에 가져와져 있는지 확인하세요.',
        fail: '이 가상 프린터의 TLS 인증서가 없습니다. Goo Buddy 데이터 디렉터리가 쓰기 가능한지 확인하세요.'
      }
    }
  }
};
