export default {
  ...
  nav: {
    ...
    logout: '退出登录',
    installApp: '安装应用',
    installAppSuccess: 'Goo Buddy 已安装',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldLabel: '维护模式',
      editFieldHelp: '启用后，此打印机将从 MQTT、队列调度和通知中暂停 — 适用于维修、并行 Goo Buddy 安装或临时停用。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: '切片机没有在打印机的 SD 卡上留下 .gcode.3mf 文件，因此 Goo Buddy 无法获取缩略图或切片机元数据。这通常是因为切片机 (Bambu Studio / OrcaSlicer 的设备选项卡) 中"将发送的文件存储在外部存储中"已关闭。',
      docsLink: '查看安装步骤 4',
      dismissLabel: '关闭此通知',
    },
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: '登录时 LDAP 组映射到 Goo Buddy 组',
    ...
    concurrentUploadsDescription: '队列可以同时向多少台打印机发送文件。打印机接收文件很慢（大型打印可能需要几分钟），并且每台都要排队等候——因此在打印机较多时，调高该值可以避免批次中最后一台打印机等完前面所有传输。如果网络或 Goo Buddy 主机难以承受并行传输，请调低该值。',
    ...
    preheatFilamentTargetsHint: 'Goo Buddy 会在已装入的 AMS 槽位中选择最高目标；仅 PLA 的打印推导为 0 并自动跳过腔体阶段。',
    ...
    authDisabledMessage: '启用身份验证以创建用户账户、管理权限并保护您的 Goo Buddy 实例。',
    ...
    disableAuthenticationMessage: '确定要禁用身份验证吗？这将使您的 Goo Buddy 实例无需登录即可访问。所有用户将保留在数据库中但身份验证将被禁用。',
    disableAuthentication: '禁用身份验证',
    // Additional settings
    configureBambuddy: '配置 Goo Buddy',
    systemDefault: '系统默认',
    ...
    captureFinishPhotoDescription: '打印完成时从打印机摄像头拍照。Goo Buddy 会在打印期间录制一段短延时摄影，以便从热床下降前的瞬间获取照片；如果您为本次打印启用了延时摄影，文件将保留，否则会在拍照完成后自动删除。',
    ...
    bambuddySoftware: 'Goo Buddy 软件',
    autoCheckDescription: '启动时自动检查新版本',
    checkNow: '立即检查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '发布说明',
    updateViaDocker: '通过 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。请在 Home Assistant 中打开 设置 → 加载项 → Goo Buddy 以安装新版本。',
    ...
    goToBackup: '前往备份',
    externalUrl: '外部URL',
    externalUrlDescription: 'Goo Buddy可访问的外部URL。用于通知图像和外部集成。',
    ...
    mqttDescription: '将Goo Buddy事件发布到外部MQTT代理，用于与Node-RED、Home Assistant和其他自动化系统集成。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP 密钥和 OIDC client_secrets 以明文存储。请设置 MFA_ENCRYPTION_KEY 或使用可写数据目录重启 Goo Buddy 以自动生成。',
      ...
      migrationErrorWarning: '{{count}} 行旧数据在启动时未能重新加密。请检查服务器日志并重启 Goo Buddy 以重试。',
    },
  ...
  hmsErrors: {
    ...
    runoutSlotUnknown: '耗材已用尽，打印已暂停。Goo Buddy 无法确定打印机现在需要哪个料槽——请在打印机屏幕上查看所请求的料槽。',
    actions: {
  ...
  login: {
    ...
    twoFA: {
      ...
      methodEmail: '邮箱验证码',
      methodBackup: '备用恢复码',
      instructionsTotp: '请打开您的身份验证器应用，输入 Goo Buddy 的 6 位验证码。',
  ...
  setup: {
    title: 'Goo Buddy 设置',
    subtitle: '为您的 Goo Buddy 实例配置身份验证',
    enableAuth: '启用身份验证',
    adminAccount: '管理员账户',
  ...
  users: {
    title: '用户管理',
    subtitle: '管理用户及其对 Goo Buddy 实例的访问',
    backToSettings: '返回设置',
    createUser: '创建用户',
    noPermission: '您没有访问此页面的权限。',
    admin: '管理员',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        ...
        description: '登录您的 Orca Cloud 账户,将切片机配置同步到 Goo Buddy。',
      },
      device: {
        title: '在 Orca Cloud 中批准 Goo Buddy',
        instruction: '打开 Orca Cloud 并批准此代码。批准后 Goo Buddy 会自动连接。',
        codeLabel: '您的配对代码',
  ...
  sponsors: {
    ...
    tagline: 'Goo Buddy 完全免费，并将持续免费——这要感谢主动支持的用户。没有风投，也没有云端绑定。',
    viewSupporters: '查看支持者',
    toastPrints: '你已经用 Goo Buddy 完成了 {{count}} 次打印。Goo Buddy 之所以免费，离不开支持者。',
    toastCost: '你已经用 Goo Buddy 追踪了 {{total}} 的耗材。看看是谁让项目保持独立。',
    toastArchives: '用 Goo Buddy 归档了 {{count}} 次打印。看看是谁让它保持独立。',
    toastAnniversary: '与 Goo Buddy 相伴一年了！看看是谁让项目保持独立。',
    toastVersionUpdate: '已更新至 v{{version}}。Goo Buddy 之所以免费，离不开支持者。',
    toastBusiness: '您正在 {{count}} 台打印机上运行 Goo Buddy？我们为团队提供支持方案：优先修复、开具发票，以及与维护者的直接沟通渠道。',
    businessCta: 'Goo Buddy 商业版',
    businessTitle: 'Goo Buddy 商业版',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'Goo Buddy 通过您的反向代理以 HTTPS 提供服务，但您的 Spoolman 地址仍为 HTTP。出于安全考虑，浏览器会阻止混合内容，因此嵌入式 Spoolman 界面无法加载。Spoolman 也必须通过 HTTPS 访问。',
    spoolmanMixedContentFixReverseProxy: '请将 Spoolman 置于与 Goo Buddy 相同的反向代理（Traefik / Nginx / Caddy）之后并启用 HTTPS，然后在设置中将 Spoolman URL 更新为新的 HTTPS 地址。',
  ...
  backup: {
    ...
    repoIsPublicWarning: '仓库为公开（PUBLIC）。Goo Buddy 备份包含 MQTT 凭据、Home Assistant 令牌、Prometheus 令牌、您的 Bambu Cloud 邮箱以及通过 K-profile 暴露的打印机访问代码。在您于提供商设置中将仓库改为私有之前，保存将被阻止。',
    repoVisibilityUnknown: '无法确定仓库的可见性。Goo Buddy 拒绝向任何未确认为私有的目标进行备份；保存将被阻止。',
    ...
    appSettings: '应用设置',
    appSettingsDescription: 'Goo Buddy 配置（完整数据库）',
    spoolInventory: '耗材库存',
    ...
    // Local Backup
    localBackup: '本地备份',
    localBackupDescription: '创建 Goo Buddy 数据的完整备份，包括数据库、档案、上传和所有文件。',
    ...
    backupRestoredRestart: '备份已恢复。请重启 Goo Buddy。',
    ...
    pathCheck: {
      title: 'Goo Buddy 无法写入该目录',
      howToFix: '解决方法：',
      sandboxed: 'Goo Buddy 服务无法写入 {{path}}。它的 systemd 单元以 ProtectSystem=strict 运行，因此除安装、数据和日志目录之外的所有目录对服务而言都是只读的，即使你自己的 shell 可以写入也一样。',
      read_only: '{{path}} 位于只读文件系统上。',
      permission_denied: 'Goo Buddy 无权写入 {{path}}。请检查该目录的属主和权限。',
      no_space: '{{path}} 所在的文件系统已满。',
      not_a_directory: '{{path}} 存在，但不是目录。',
      missing: '{{path}} 不存在且无法创建。',
      error: 'Goo Buddy 无法写入 {{path}}。',
      ephemeralTitle: '这些备份在容器重建后会丢失',
      container_ephemeral: '{{path}} 位于 Goo Buddy 容器内部，而不是宿主机上。写入其中的备份会在容器重建时丢失。请从宿主机挂载该目录：',
    },
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '备注保存在 Goo Buddy 中，不在打印机上',
      syncing: '与打印机同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '启用虚拟打印机',
      visibleInSlicer: '在切片软件发现中显示为"Goo Buddy"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未激活',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自动检测（默认）...',
      hint: '覆盖通过 SSDP 广播并在 TLS 证书中使用的 IP 地址。在 Goo Buddy 有多个网络接口时很有用。',
    },
    mode: {
      title: '模式',
      archive: '归档',
    ...
    caCert: {
      title: '切片软件证书',
      description: '虚拟打印机使用由 Goo Buddy CA 签发的 TLS 证书。将此 CA 证书导入切片软件的信任库一次，切片软件即可接受连接 — 无需再通过命令行获取。',
      copy: '复制',
      copied: '已复制',
      download: '下载',
      fingerprint: 'SHA-256',
    ...
    howItWorks: {
      ...
      step2: '在归档、审核和队列模式下，使用切片软件中的"发送"按钮将 3MF 文件上传到 Goo Buddy。切片软件会显示"打印成功"— 文件已存储，未打印。',
  ...
  smartPlugs: {
    ...
    restEnergyTotalHint:
      '许多插座（包括所有 Shelly）只提供永不归零的累计计数值。该值应填在此处，而不是上面的字段：若当作今日用量读取，它在午夜不会归零，而“昨日”和“总计”会一直为空。Goo Buddy 会据此推算“今日”和“昨日”，这需要一到两天的采样数据。Shelly 以瓦时为单位，因此倍数请填 0.001。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'Goo Buddy 正在以 Docker bridge 网络运行。打印机发现和虚拟打印机需要 host 网络模式 — 请使用 "network_mode: host" 重新创建容器。',
        skip: '未在 Docker 中运行 — 不适用。',
      },
      subnet: {
        title: '网络子网',
        pass: '打印机和 Goo Buddy 位于同一子网。',
        warn: '打印机（{{printer_ip}}）和 Goo Buddy（{{host_ip}}）位于不同的子网。除非配置了子网之间的路由，否则它们可能无法相互访问。',
        skip: '无法确定子网 — 已跳过。',
      },
      mqtt_auth: {
        title: '打印机凭据',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: '从打印机屏幕（局域网设置）重新复制访问码，并在 Goo Buddy 的打印机设置中更新它。',
      },
      'ftp-connection-timeout': {
        name: '文件传输连接超时',
        cause: 'Goo Buddy 无法连接打印机的文件传输端口（FTPS 990）。该端口被阻止，或打印机已关机或位于其他子网。',
        fix: '请确保 Goo Buddy 与打印机之间的 990 端口未被阻止，并且两者位于同一网络。',
      },
      'ftp-ssl-error': {
        name: '安全文件传输握手失败',
      ...
      'database-locked': {
        ...
        fix: '请将 Goo Buddy 切换到外部 PostgreSQL 数据库。参见文档中的 PostgreSQL 指南。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: '服务正在运行',
        fail: '虚拟打印机已启用，但其服务未运行。请检查 Goo Buddy 日志 — 通常是绑定 IP 冲突或权限错误使其停止。',
      },
      bind_interface: {
        title: '绑定网络接口',
      ...
      certificate: {
        title: 'TLS 证书',
        pass: '证书已就绪。请确保已将 Goo Buddy CA 证书（上方）导入切片软件的信任库。',
        fail: '此虚拟打印机的 TLS 证书缺失。请检查 Goo Buddy 数据目录是否可写。',
      },
    },
  },
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML API 通过 URL 获取摄像头快照。请在通用设置中设置外部 URL，以便 ML API 容器可以访问 Goo Buddy。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: '粘贴 MakerWorld 模型链接，即可直接在 Goo Buddy 中导入并打印 —— 无需切换到 Bambu Handy 应用。',
    ...
    signInExpiredTitle: 'Bambu Cloud 登录已过期',
    signInExpiredBody: '您仍处于 Goo Buddy 的登录状态，但 Bambu Lab 已不再接受已保存的令牌，因此下载会失败。请重新登录 Bambu Cloud。',
    ...
    disclaimer: 'MakerWorld 集成使用由社区记录的 API 接口。Goo Buddy 与 MakerWorld 或 Bambu Lab 没有从属或认可关系。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（链接）文件夹中的文件将被跳过 — Goo Buddy 从不删除不属于自己的字节。',    previewLoading: '正在检查匹配的文件数量…',
