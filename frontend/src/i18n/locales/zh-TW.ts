export default {
  ...
  nav: {
    ...
    logout: '登出',
    installApp: '安裝應用程式',
    installAppSuccess: 'Goo Buddy 已安裝',
  },
  ...
  printers: {
    ...
    maintenance: {
      ...
      editFieldLabel: '維護模式',
      editFieldHelp: '啟用後，此印表機將從 MQTT、佇列調度和通知中暫停 — 適用於維修、並行 Goo Buddy 安裝或暫時停用。',
    },
    // Chamber light
  ...
  archives: {
    ...
    no3mfBanner: {
      ...
      body: '切片機沒有在印表機的 SD 卡上留下 .gcode.3mf 檔案，因此 Goo Buddy 無法取得縮圖或切片機中繼資料。這通常是因為切片機 (Bambu Studio / OrcaSlicer 的裝置分頁) 中「將傳送的檔案儲存在外部儲存中」已關閉。',
      docsLink: '檢視安裝步驟 4',
      dismissLabel: '關閉此通知',
    },
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: '登入時 LDAP 群組對應到 Goo Buddy 群組',
    ...
    concurrentUploadsDescription: '佇列可同時向多少台印表機傳送檔案。印表機接收檔案的速度很慢（大型列印可能需要數分鐘），而且每台都得排隊等候——因此在印表機較多時，調高此值可避免批次中最後一台印表機等完前面所有傳輸。若網路或 Goo Buddy 主機難以負荷並行傳輸，請調低此值。',
    ...
    preheatFilamentTargetsHint: 'Goo Buddy 在已裝入的 AMS 槽位中挑選最高目標；只有 PLA 的列印推導為 0，並自動跳過腔體階段。',
    ...
    authDisabledMessage: '啟用身份驗證以建立使用者帳戶、管理權限並保護您的 Goo Buddy 實例。',
    ...
    disableAuthenticationMessage: '確定要停用身份驗證嗎？這將使您的 Goo Buddy 實例無需登入即可存取。所有使用者將保留在資料庫中但身份驗證將被停用。',
    disableAuthentication: '停用身份驗證',
    // Additional settings
    configureBambuddy: '設定 Goo Buddy',
    systemDefault: '系統預設',
    ...
    captureFinishPhotoDescription: '列印完成時從印表機攝影機拍照。Goo Buddy 會在列印期間錄製一段短縮時攝影，以便從熱床下降前的瞬間取得照片；如果您為本次列印啟用了縮時攝影，檔案將保留，否則會在拍照完成後自動刪除。',
    ...
    bambuddySoftware: 'Goo Buddy 軟體',
    autoCheckDescription: '啟動時自動檢查新版本',
    checkNow: '立即檢查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '發布說明',
    updateViaDocker: '透過 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。請在 Home Assistant 中開啟 設定 → 附加元件 → Goo Buddy 以安裝新版本。',
    ...
    goToBackup: '前往備份',
    externalUrl: '外部URL',
    externalUrlDescription: 'Goo Buddy可存取的外部URL。用於通知影像和外部整合。',
    ...
    mqttDescription: '將Goo Buddy事件發布到外部MQTT代理，用於與Node-RED、Home Assistant和其他自動化系統整合。',
    ...
    encryption: {
      ...
      notConfiguredDesc: 'TOTP 機密和 OIDC client_secrets 以明文儲存。請設定 MFA_ENCRYPTION_KEY 或使用可寫資料目錄重新啟動 Goo Buddy 以自動產生。',
      ...
      migrationErrorWarning: '{{count}} 行舊資料在啟動時未能重新加密。請檢查伺服器日誌並重新啟動 Goo Buddy 以重試。',
    },
  ...
  hmsErrors: {
    ...
    runoutSlotUnknown: '耗材已用盡，列印已暫停。Goo Buddy 無法確定印表機現在需要哪個料槽——請在印表機螢幕上查看所請求的料槽。',
    actions: {
  ...
  login: {
    ...
    twoFA: {
      ...
      methodEmail: '信箱驗證碼',
      methodBackup: '備用恢復碼',
      instructionsTotp: '請開啟您的身份驗證器 App，輸入 Goo Buddy 的 6 位驗證碼。',
  ...
  setup: {
    title: 'Goo Buddy 設定',
    subtitle: '為您的 Goo Buddy 實例設定身份驗證',
    enableAuth: '啟用身份驗證',
    adminAccount: '管理員帳戶',
  ...
  users: {
    title: '使用者管理',
    subtitle: '管理使用者及其對 Goo Buddy 實例的存取',
    backToSettings: '返回設定',
    createUser: '建立使用者',
    noPermission: '您沒有存取此頁面的權限。',
    admin: '管理員',
  ...
  profiles: {
    ...
    orcaCloud: {
      ...
      connect: {
        ...
        description: '登入您的 Orca Cloud 帳號,將切片機設定檔同步到 Goo Buddy。',
      },
      device: {
        title: '在 Orca Cloud 中核准 Goo Buddy',
        instruction: '開啟 Orca Cloud 並核准此代碼。核准後 Goo Buddy 會自動連接。',
        codeLabel: '您的配對代碼',
  ...
  sponsors: {
    ...
    tagline: 'Goo Buddy 完全免費，並將持續免費——感謝主動支持的使用者。沒有創投，也沒有雲端綁定。',
    viewSupporters: '查看支持者',
    toastPrints: '你已經用 Goo Buddy 完成了 {{count}} 次列印。Goo Buddy 之所以免費，要感謝支持者。',
    toastCost: '你已經用 Goo Buddy 追蹤了 {{total}} 的耗材。看看是誰讓專案保持獨立。',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'Goo Buddy 透過您的反向代理以 HTTPS 提供服務，但您的 Spoolman 位址仍為 HTTP。基於安全考量，瀏覽器會封鎖混合內容，因此內嵌的 Spoolman 介面無法載入。Spoolman 也必須可透過 HTTPS 存取。',
    spoolmanMixedContentFixReverseProxy: '請將 Spoolman 置於與 Goo Buddy 相同的反向代理（Traefik / Nginx / Caddy）之後並啟用 HTTPS，然後在設定中將 Spoolman URL 更新為新的 HTTPS 位址。',
  ...
  backup: {
    ...
    repoIsPublicWarning: '儲存庫為公開（PUBLIC）。Goo Buddy 備份包含 MQTT 認證、Home Assistant 權杖、Prometheus 權杖、您的 Bambu Cloud 電子郵件，以及透過 K-profile 暴露的印表機存取碼。在您於服務商設定中將儲存庫改為私有之前，儲存將被阻擋。',
    repoVisibilityUnknown: '無法確認儲存庫的可見性。Goo Buddy 拒絕向任何未確認為私有的目標進行備份；儲存將被阻擋。',
    ...
    appSettings: '應用程式設定',
    appSettingsDescription: 'Goo Buddy 設定（完整資料庫）',
    spoolInventory: '耗材庫存',
    ...
    // Local Backup
    localBackup: '本機備份',
    localBackupDescription: '建立 Goo Buddy 資料的完整備份，包括資料庫、檔案、上傳和所有檔案。',
    ...
    backupRestoredRestart: '備份已恢復。請重新啟動 Goo Buddy。',
    ...
    pathCheck: {
      title: 'Goo Buddy 無法寫入此目錄',
      howToFix: '解決方法：',
      sandboxed: 'Goo Buddy 服務無法寫入 {{path}}。其 systemd unit 以 ProtectSystem=strict 執行，因此安裝、資料與記錄目錄以外的所有目錄對服務而言都是唯讀的，即使你自己的 shell 可以寫入也一樣。',
      read_only: '{{path}} 位於唯讀檔案系統上。',
      permission_denied: 'Goo Buddy 沒有寫入 {{path}} 的權限。請檢查該目錄的擁有者與權限。',
      no_space: '{{path}} 所在的檔案系統已滿。',
      not_a_directory: '{{path}} 存在，但不是目錄。',
      missing: '{{path}} 不存在且無法建立。',
      error: 'Goo Buddy 無法寫入 {{path}}。',
      ephemeralTitle: '這些備份在容器重建後會遺失',
      container_ephemeral: '{{path}} 位於 Goo Buddy 容器內，而非主機上。寫入其中的備份會在容器重建時遺失。請從主機掛載該目錄：',
    },
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '備註儲存在 Goo Buddy 中，不在印表機上',
      syncing: '與印表機同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '啟用虛擬印表機',
      visibleInSlicer: '在切片軟體發現中顯示為"Goo Buddy"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未啟用',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自動檢測（預設）...',
      hint: '覆蓋透過 SSDP 廣播並在 TLS 憑證中使用的 IP 位址。在 Goo Buddy 有多個網路介面時很有用。',
    },
    mode: {
      title: '模式',
      archive: '歸檔',
    ...
    caCert: {
      title: '切片軟體憑證',
      description: '虛擬印表機使用由 Goo Buddy CA 簽發的 TLS 憑證。將此 CA 憑證匯入切片軟體的信任庫一次，切片軟體即可接受連線 — 無需再透過命令列取得。',
      copy: '複製',
      copied: '已複製',
      download: '下載',
      fingerprint: 'SHA-256',
    ...
    howItWorks: {
      ...
      step2: '在歸檔、審核和佇列模式下，使用切片軟體中的"傳送"按鈕將 3MF 檔案上傳到 Goo Buddy。切片軟體會顯示"列印成功"— 檔案已儲存，未列印。',
  ...
  smartPlugs: {
    ...
    restEnergyTotalHint:
      '許多插座（包括所有 Shelly）只提供永不歸零的累計計數值。該值應填在此處，而非上方欄位：若當作今日用量讀取，它在午夜不會歸零，而「昨日」與「總計」會一直是空的。Goo Buddy 會據此推算「今日」與「昨日」，這需要一至兩天的取樣資料。Shelly 以瓦時為單位，因此倍數請填 0.001。',
  ...
  diagnostic: {
    ...
    check: {
      ...
      network_mode: {
        ...
        warn: 'Goo Buddy 正在以 Docker bridge 網路執行。印表機探索與虛擬印表機需要 host 網路模式 — 請使用 "network_mode: host" 重新建立容器。',
        skip: '未在 Docker 中執行 — 不適用。',
      },
      subnet: {
        title: '網路子網路',
        pass: '印表機與 Goo Buddy 位於同一子網路。',
        warn: '印表機（{{printer_ip}}）與 Goo Buddy（{{host_ip}}）位於不同的子網路。除非設定了子網路之間的路由，否則它們可能無法互相連線。',
        skip: '無法判定子網路 — 已略過。',
      },
      mqtt_auth: {
        title: '印表機認證資訊',
  ...
  systemHealth: {
    ...
    signature: {
      'ftp-auth-rejected': {
        ...
        fix: '從印表機螢幕（區域網路設定）重新複製存取碼，並在 Goo Buddy 的印表機設定中更新。',
      },
      'ftp-connection-timeout': {
        name: '檔案傳輸連線逾時',
        cause: 'Goo Buddy 無法連線印表機的檔案傳輸連接埠（FTPS 990）。該連接埠被封鎖，或印表機已關機或位於其他子網路。',
        fix: '請確認 Goo Buddy 與印表機之間的 990 連接埠未被封鎖，且兩者位於同一網路。',
      },
      'ftp-ssl-error': {
        name: '安全檔案傳輸交握失敗',
      ...
      'database-locked': {
        ...
        fix: '請將 Goo Buddy 切換到外部 PostgreSQL 資料庫。參見文件中的 PostgreSQL 指南。',
      },
    },
  },
  ...
  vpDiagnostic: {
    ...
    check: {
      ...
      running: {
        title: '服務正在執行',
        fail: '虛擬印表機已啟用，但其服務未執行。請檢查 Goo Buddy 記錄 — 通常是繫結 IP 衝突或權限錯誤使其停止。',
      },
      bind_interface: {
        title: '繫結網路介面',
      ...
      certificate: {
        title: 'TLS 憑證',
        pass: '憑證已就緒。請確保已將 Goo Buddy CA 憑證（上方）匯入切片軟體的信任庫。',
        fail: '此虛擬印表機的 TLS 憑證遺失。請檢查 Goo Buddy 資料目錄是否可寫入。',
      },
    },
  },
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML API 透過 URL 擷取攝影機快照。請在一般設定中設定外部 URL，讓 ML API 容器可以連線到 Goo Buddy。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: '貼上 MakerWorld 模型連結，即可直接在 Goo Buddy 中匯入並列印 —— 無需切換至 Bambu Handy 應用程式。',
    ...
    signInExpiredTitle: 'Bambu Cloud 登入已過期',
    signInExpiredBody: '您仍處於 Goo Buddy 的登入狀態，但 Bambu Lab 已不再接受已儲存的權杖，因此下載會失敗。請重新登入 Bambu Cloud。',
    ...
    disclaimer: 'MakerWorld 整合使用由社群記錄的 API 介面。Goo Buddy 與 MakerWorld 或 Bambu Lab 無從屬或認可關係。',
  ...
  libraryPurge: {
    ...
    effect4: '外部（連結）資料夾中的檔案將被略過 — Goo Buddy 從不刪除不屬於自己的位元組。',    previewLoading: '正在檢查符合的檔案數量…',
