...
class TestGetDesign:
    ...
    @pytest.mark.asyncio
    async def test_sends_honest_goo_buddy_user_agent(self, service):
        """The client identifies honestly as Goo Buddy, not as Firefox.

        Earlier iterations of this code stripped ``x-bbl-*`` Bambu-app
        identification headers but kept a Firefox User-Agent. Verified
        2026-05-12 that MakerWorld treats the Goo Buddy application user agent identically to
