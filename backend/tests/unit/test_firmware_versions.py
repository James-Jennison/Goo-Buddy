...
async def test_client_headers_identify_honestly_and_send_browser_accept():
    ...
    must identify as Goo Buddy at the HTTP layer and must send Accept +
...
async def test_bambulab_curl_cffi_session_keeps_honest_user_agent():
    ...
    HTTP-layer User-Agent MUST stay 'Goo-Buddy/...'. A future refactor that
    ...
    assert ua.startswith("Goo-Buddy/"), f"curl_cffi session UA leaked Chrome default: {ua!r}"
