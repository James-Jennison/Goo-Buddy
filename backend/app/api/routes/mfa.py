...
async def setup_totp(
    ...
    provisioning_uri = totp.provisioning_uri(name=current_user.username, issuer_name="Goo Buddy")
    ...
    return TOTPSetupResponse(secret=secret, qr_code_b64=qr_b64, issuer="Goo Buddy")
...
async def enable_email_otp(
    ...
    try:
        send_email(
            ...
            subject="Verify your Goo Buddy email address for 2FA",
            body_text=(
                f"Your Goo Buddy email 2FA setup code is: {code}\n\n"
                "Enter this code to confirm email-based two-factor authentication.\n"
                "The code expires in 10 minutes."
            ),
            body_html=(
                "<p>To enable <strong>email-based two-factor authentication</strong> on your Goo Buddy account, "
...
async def send_email_otp(
    ...
    try:
        send_email(
            smtp_settings=smtp_settings,
            to_email=user.email,
            subject="Your Goo Buddy verification code",
            body_text=f"Your Goo Buddy login code is: {code}\n\nThis code expires in {UserOTPCode.OTP_TTL_MINUTES} minutes and can only be used once.",
            body_html=(
                f"<p>Your <strong>Goo Buddy</strong> login verification code is:</p>"
                f"<h2 style='letter-spacing:4px'>{code}</h2>"
