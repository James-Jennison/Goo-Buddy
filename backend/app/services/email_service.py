...
async def get_smtp_settings(db: AsyncSession) -> SMTPSettings | None:
    ...
    return SMTPSettings(
        ...
        smtp_from_name=settings_dict.get("smtp_from_name", "Goo Buddy"),
    )
...
def create_welcome_email(username: str, password: str, login_url: str) -> tuple[str, str, str]:
    ...
    """
    subject = "Welcome to Goo Buddy - Your Account Details"

    text_body = f"""Welcome to Goo Buddy!
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); background-color: #667eea; padding: 20px; border-radius: 8px 8px 0 0;">
        <h1 style="color: #ffffff; margin: 0; font-size: 24px; text-shadow: 0 1px 2px rgba(0,0,0,0.3);">Welcome to Goo Buddy!</h1>
    </div>
...
def create_password_reset_email(username: str, password: str, login_url: str) -> tuple[str, str, str]:
    ...
    Returns:
        Tuple of (subject, text_body, html_body)
    """
    subject = "Goo Buddy - Your Password Has Been Reset"

    text_body = f"""Your Goo Buddy password has been reset.
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; border: 1px solid #ddd; border-top: none;">
        <p style="font-size: 16px;">Your Goo Buddy password has been reset.</p>
...
def create_password_reset_link_email(username: str, reset_url: str) -> tuple[str, str, str]:
    ...
    subject = "Goo Buddy - Password Reset Request"

    text_body = f"""A password reset was requested for your Goo Buddy account.
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; border: 1px solid #ddd; border-top: none;">
        <p style="font-size: 16px;">A password reset was requested for your Goo Buddy account (<strong>{username}</strong>).</p>
        ...
        <p style="font-size: 14px; color: #999; margin-top: 30px;">
            Best regards,<br>Goo Buddy Team
        </p>
    </div>
</body>
</html>
...
async def create_password_reset_link_email_from_template(
    ...
    if template:
        ...
        subject = render_template(template.subject or "Goo Buddy - Password Reset Request", variables)
...
async def create_welcome_email_from_template(
    db: AsyncSession, username: str, password: str, login_url: str, app_name: str = "Goo Buddy"
) -> tuple[str, str, str]:
