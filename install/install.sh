#!/usr/bin/env bash
#
# Goo Buddy Native Installation Script
# Supports: Debian/Ubuntu, RHEL/Fedora/CentOS, Arch Linux, macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/James-Jennison/Goo-Buddy/main/install/install.sh -o install.sh && chmod +x install.sh && ./install.sh
...
show_help() {
    echo "Goo Buddy Native Installation Script"
    echo ""
...
download_bambuddy() {
    log_info "Downloading Goo Buddy..."

    # Validate branch exists on remote before proceeding
    if ! git ls-remote --exit-code --heads https://github.com/James-Jennison/Goo-Buddy.git "$BRANCH" &>/dev/null; then
        log_error "Branch '$BRANCH' not found in the Goo Buddy repository."
        log_info "Available branches:"
        git ls-remote --heads https://github.com/James-Jennison/Goo-Buddy.git | sed 's|.*refs/heads/|  - |'
        exit 1
    fi
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        if [[ -d "$INSTALL_PATH/.git" ]]; then
            ...
            git clone --branch "$BRANCH" https://github.com/James-Jennison/Goo-Buddy.git "$INSTALL_PATH"
        fi
        ...
        sudo git clone --branch "$BRANCH" https://github.com/James-Jennison/Goo-Buddy.git "$INSTALL_PATH"
        sudo chown -R "$SERVICE_USER:$SERVICE_USER" "$INSTALL_PATH" 2>/dev/null || true
    fi

    log_success "Goo Buddy downloaded to $INSTALL_PATH (branch: $BRANCH)"
}
...
[Unit]
Description=Goo Buddy - Bambu Lab Print Management
Documentation=https://github.com/James-Jennison/Goo-Buddy
...
EOF
    ...
    if prompt_yes_no "Enable Goo Buddy to start on boot?" "y"; then
        sudo systemctl enable bambuddy
        log_success "Service enabled"
    fi

    if prompt_yes_no "Start Goo Buddy now?" "y"; then
        sudo systemctl start bambuddy
        sleep 2
        if sudo systemctl is-active --quiet bambuddy; then
            log_success "Goo Buddy is running"
        else
...
EOF
    ...
    if prompt_yes_no "Load Goo Buddy service now?" "y"; then
...
main() {
    ...
    if [[ "$BIND_ADDRESS" == "0.0.0.0" ]]; then
        ...
        echo -e "  ${BOLD}Access Goo Buddy:${NC}  ${CYAN}http://localhost:$PORT${NC}"
    ...
    echo ""
    echo -e "  ${BOLD}Update Goo Buddy:${NC}"
    ...
    echo ""
    echo -e "  ${BOLD}Documentation:${NC}  ${CYAN}https://github.com/James-Jennison/Goo-Buddy#readme${NC}"
    echo ""
}
