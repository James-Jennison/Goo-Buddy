#!/usr/bin/env bash
#
# Goo Buddy Docker Installation Script
# Supports: Linux (all distros), macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/James-Jennison/Goo-Buddy/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
...
show_help() {
    echo "Goo Buddy Docker Installation Script"
    echo ""
...
download_compose_file() {
    ...
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
        ...
        curl -fsSL -o docker-compose.yml \
            https://raw.githubusercontent.com/James-Jennison/Goo-Buddy/main/docker-compose.yml
    fi
...
create_env_file() {
    ...
    cat > .env << EOF
# Goo Buddy Docker Configuration
# Generated by docker-install.sh on $(date)

# Port Goo Buddy runs on
PORT=$PORT
...
start_container() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "Goo Buddy container is running"
            return 0
        fi
