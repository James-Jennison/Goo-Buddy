...
<p align="center">
  ...
  Raspberry Pi-friendly, local-first printer management for FDM printers.
</p>
...
> [GNU AGPL v3.0](LICENSE). See [the architecture note](docs/GOO_BUDDY_ARCHITECTURE.md)
> and [upstream-sync policy](docs/UPSTREAM_SYNC.md).

<p align="center">
  <a href="https://github.com/James-Jennison/Goo-Buddy"><img src="https://img.shields.io/github/license/James-Jennison/Goo-Buddy?style=flat-square&cacheSeconds=3600" alt="AGPL-3.0 License"></a>
  <a href="https://github.com/James-Jennison/Goo-Buddy/issues"><img src="https://img.shields.io/github/issues/James-Jennison/Goo-Buddy?style=flat-square&cacheSeconds=3600" alt="Issues"></a>
</p>

<p align="center">
  <a href="https://github.com/James-Jennison/Goo-Buddy"><strong>Repository</strong></a> •
...
## 📰 Upstream Bambuddy press coverage (historical)
...
**No desktop slicer required.** Drop an STL or 3MF into Goo Buddy's File Manager, hit **Slice**, and the result lands as a ready-to-print `.gcode.3mf` in the same folder — without ever opening Bambu Studio or Orca Slicer.
...
- 📱 **Slice from your phone or tablet** — Goo Buddy's PWA + the new server-side slicer means you can drop an STL in from mobile and queue a print without ever touching a desktop.
...
- 🔄 **Re-slice for a different printer in one click** — Open any sliced archive in Goo Buddy and re-slice it for any printer, including across the single-nozzle ↔ dual-nozzle (H2D / H2D Pro) boundary that BambuStudio's CLI would normally reject. Goo Buddy detects the class change and auto-arranges objects laid out for the source bed (e.g. X1C 256×256) so they land safely on the target (e.g. H2D 350×320 with its per-nozzle dead zones).
...
- 🔁 **Same dispatch as the rest of Goo Buddy** — The sliced output flows into the existing queue / plate-picker / AMS-mapping path, so all the regular conveniences (multi-printer dispatch, AMS routing, scheduled prints) just work.

Optional but recommended — drop the [`slicer-api/` Compose stack](slicer-api/README.md) next to your Goo Buddy install and the **Slice** button lights up everywhere.
...
- Live camera streaming (MJPEG) & snapshots with multi-viewer support — most Bambu printers only allow one upstream connection, so Goo Buddy fans out a single shared stream to all browser tabs / cards / overlays
...
- **Heater history charts** — Goo Buddy logs nozzle, bed, and chamber readings every minute and surfaces them via a tiny chart icon on each heater tile in the printer card. Click for a per-heater modal with current / average / min / max stats, target overlay, and a 6h / 24h / 48h / 7d time range — works on read-only chamber sensors (X1C / P2S) too. AMS humidity and temperature get the same treatment (already shipped).
...
- Paste any `makerworld.com/models/…` URL → preview, plate picker, and import without leaving Goo Buddy
...
- **Orca Cloud profile sync** — read your OrcaSlicer 2.4.0+ cloud-synced profiles directly in Goo Buddy, usable for slicing alongside Bambu Cloud / local / standard presets. Four sign-in providers (Google / Apple / GitHub / email+password)
...
- **Single Sign-On (OIDC / SSO)** — Log in via PocketID, Authentik, Keycloak, or any standards-compliant OIDC provider. PKCE (S256) for public clients, `email_verified` gating, issuer & `aud`/`nonce` validation, opt-in account linking via verified email, optional auto-provisioning of new Goo Buddy accounts, and strict SSRF hardening on every URL pulled from the OIDC discovery document (scheme + private/loopback/link-local IP checks).
...
<p align="center">
  ...
  <br>
  <em>Spin up your own private Goo Buddy with simulated printers and pre-loaded print history. Click around freely — it's your sandbox. ~10 seconds to spawn, 30-minute session, no signup.</em>
</p>
...
<p align="center">
  <a href="https://youtu.be/bmq2Z0lEXeo">
    <img src="https://img.youtube.com/vi/bmq2Z0lEXeo/maxresdefault.jpg" alt="Goo Buddy Demo Video" width="800">
  </a>
...
- Docker with Buildx for container builds (recommended on Raspberry Pi)
...
#### Windows (upstream installer compatibility)

The inherited upstream Windows installer is not yet branded or released for Goo Buddy.
Use a source build below rather than downloading an upstream Goo Buddy artifact.

#### Docker (Linux / macOS / Windows via Docker Desktop)

**Build from source (current Goo Buddy path)**
```bash
...
> **Raspberry Pi 4/5:** use the explicit [ARM64 Buildx command](docs/CONTAINER_BUILDS.md#raspberry-pi-arm64-validation). It resolves ARM64 base images freshly and does not publish an image.
...
| `PORT` | `8000` | Port Goo Buddy runs on (with host networking mode) |
...
> Goo Buddy does not publish a container image as part of this foundation. Build
> and validate locally until a separately authorized release process exists.
...
```yaml
services:
  bambuddy: # compatibility-preserved Compose service key
    build: .
    network_mode: host
```

> **Note:** Docker's default bridge networking cannot receive SSDP multicast packets for automatic printer discovery. When using `network_mode: host`, Goo Buddy auto-detects your network subnet and can discover printers via subnet scanning in the Add Printer dialog.
...
> **Need implementation detail?** See the [repository documentation](docs/) and
> [container build guide](docs/CONTAINER_BUILDS.md).
...
> The inherited script retains Goo Buddy-named compatibility paths and has not
> been qualified as a Goo Buddy installer. Prefer the Docker source build above.

### Enabling Developer Mode

Developer Mode enables the existing Bambu integration to control a printer over
the local network. It is not used by the synthetic Elegoo foundation.
...
For the existing Bambu integration, enable **"Store sent files on external storage"** so that print files (3MF) are saved to the printer's SD card. Goo Buddy needs these files to extract thumbnails and 3D model previews.
...
Goo Buddy documentation lives in this repository:

- [Architecture and platform status](docs/GOO_BUDDY_ARCHITECTURE.md)
- [Safe upstream synchronization](docs/UPSTREAM_SYNC.md)
- [Container builds for native, ARM64, and release validation](docs/CONTAINER_BUILDS.md)
- [Repository documentation directory](docs/)
...
Not sure where to start? Open a [Goo Buddy issue](https://github.com/James-Jennison/Goo-Buddy/issues) with the area you want to help improve.

### 🔒 Looking for a security-focused contributor

I'm bringing on a contributor whose specific focus is keeping an eye on Goo Buddy's security.
...
No fixed time commitment. If you're interested, open a Goo Buddy issue describing your security-review interests.
...
Goo Buddy remains licensed under AGPL-3.0. It is a derivative fork of
[Bambuddy](https://github.com/maziggy/bambuddy); retain the upstream license,
notices, and attribution. See [LICENSE](LICENSE) for the full terms.
...
Goo Buddy has no separate sponsorship or commercial-support program in this
initial fork. Upstream Bambuddy sponsorship information remains historical and
is not a support channel for Goo Buddy.
...
<p align="center">
  ...
  <br><br>
  <a href="https://github.com/James-Jennison/Goo-Buddy/issues">Report Bug</a> •
  <a href="https://github.com/James-Jennison/Goo-Buddy/issues">Request Feature</a> •
  <a href="https://github.com/James-Jennison/Goo-Buddy/tree/main/docs">Documentation</a>
</p>
